...
const UpdateStaff = ({ id, data }) => {
  ...
  const onFinish = async (values) => {
    try {
      ...
      const resp = await dispatch(updateStaff({ id, values: uppercaseValues }));
      console.log(resp);
      if (resp.payload.message === "success") {
        await dispatch(loadAllStaff({ status: true, page: 1, count: 1000 }));
        setLoading(false);
        form.resetFields();
  ...
  const [initValues, setInitValues] = useState({
    ...
    helperActive: data?.driverActive,
