...
import AddPartyBusListDrawer from "../entry/proformaInvoice/addPartyBusListDrawer";
import UserPrivateComponent from "../PrivacyComponent/UserPrivateComponent";
import TableNoPagination from "../CommonUi/TableNoPagination";
import { deleteLocalProforma } from "../../redux/rtk/features/localProformaInvoice/localProformaSlice";
// import UpdatePartyBusListDrawer from "../entry/proformaInvoice/updatePartyBusListDrawer";
import dayjs from "dayjs";
import UpdateProformalInvoiceEntry from "./UpdateProformalInvoiceEntry";
...
const AddProformaInvoice = () => {
  ...
  // Add States
  const { loading } = useSelector((state) => state.localProforma);
  ...
  const onClose = (payload) => {
    ...
    setBookingArray((prevBookingArray) => {
      const index = prevBookingArray.findIndex(
        (item) =>
          item.BusTypeID === values.BusTypeID &&
          item.sittingCapacity === values.sittingCapacity
      );
      if (index !== -1) {
        console.log("My Index Is: ", index);
        const newArray = [...prevBookingArray]; // Create a copy of the array
        newArray[index] = values; // Update the specific element

        return newArray; // Return the updated array
      } else {
        // Item not found, add it to the array
        return [...prevBookingArray, values];
      }
    });
    console.log("THis Is ONclose", payload);
  };
  ...
  useEffect(() => {
    ...
    console.log("Bus Array", bookingArray);
  ...
  useEffect(() => {
    ...
    totalCalculator();
  }, [initValues, form, localProforma, performa]);
  ...
  const columns = [
    ...
    },
    {
      ...
      key: "reportTime",
      render: (text) => {
        // Parse the time string and format it to 12-hour format with AM/PM
        const formattedTime = dayjs(text, "HH:mm:ss", true);
        if (formattedTime.isValid()) {
          return formattedTime.format("hh:mm a"); // 12-hour format with AM/PM
        }
