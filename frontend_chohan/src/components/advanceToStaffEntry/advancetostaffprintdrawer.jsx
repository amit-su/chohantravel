...
function Advancetostaffprintdrawer() {
  const { companyId, AdvanceNo } = useParams();
  const data1=companyId;

  console.log(AdvanceNo,"iv")
  //Date issue//s
  ...
  useEffect(() => {
    if (!data) {
      dispatch(
        loadPowerbiToken({ reportId: "79bbee6b-6fe8-4ac5-bccf-754c94c26c87" })
      );
    }
    if (data) {
      setReportInfo(data);
  ...
  return (
    <>
      <div className="text-center">
        <div className="">
         <h1><strong>AdvanceTostaff</strong></h1>
        </div>
        <div>
      {reportInfo && data && reportInfo.embedUrl ? (
        <PowerBIEmbed
          embedConfig={{
            type: "report",
            id: reportInfo.id,
            embedUrl: reportInfo.embedUrl+"&rp:AdvanceNo="+AdvanceNo+"&rp:CompanyID="+data1,
...
export default Advancetostaffprintdrawer;
