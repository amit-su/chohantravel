...
const AddInvoice = () => {
  ...
  console.log(performa, "local perfe");
    ...
    try {
      ...
      if (bookingArray.length >= 0) {
        ...
        if (resp.data.status == 1) {
          setLoader(false);
          toast.success("Invoice Entry successfully.");
  ...
  const onBookingClose = (payload) => {
    ...
    for (let i = 0; i < performa.length && c < performa.length; i++) {
      console.log(performa[i].BookingID);
  ...
  console.log(bi, "878");
  const PartySelect = ({ partyList }) => {
    // Create a shallow copy of the array and then sort it
    const sortedPartyList = [...partyList].sort((a, b) =>
