...
const gateAllBusStock = async (req, res) => {
  try {
    const { InputBusTypeID, InputStartDateTime, InputEndDateTime } = req.body;
    ...
    const data = await databaseService.callStoredProcedure(
      req,
      GET_BUS_STOCK,
      params
    );
...
const gateAllBookingInfo = async (req, res) => {
  try {
    const params = {};
    const data = await databaseService.callStoredProcedure(
      req,
...
const gateAllexpireDocument = async (req, res) => {
  try {
    const params = {};
    const data = await databaseService.callStoredProcedure(
      req,
